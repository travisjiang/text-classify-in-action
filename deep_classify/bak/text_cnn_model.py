...
def cnn_embedding(embed_shape, embed_matrix, name_scope=""):
    ...
    with tf.name_scope(name_scope):  # embedding matrix
        ...
        ts_embedding = tf.constant(
            embed_matrix, dtype=tf.float32)  # convert to tensor


def cnn_ensemble(ts_logits_list):
...
def cnn_loss(input_y, logits, l2_lambda=0.0001, name_scope="loss"):  # 0.001
    with tf.name_scope(name_scope):
        # input: `logits`:[batch_size, class_num], and `labels`:[batch_size]
...
def cnn_inference(ts_embedding,
    ...
    with tf.name_scope(name_scope):
        ...
        for i, filter_size in enumerate(filter_sizes):
            ...
            # shape:[batch_size, 1, 1, filter_num]
...
# for conv layer
        ...
        self.filter_num_total = self.filter_num * len(filter_sizes)
# for training
...
# for flags
        ...
        self.input_x = tf.placeholder(
            tf.int32, [None, self.sentence_length], name="input_x")  # X
        self.input_y = tf.placeholder(
            tf.int32, [None, ], name="input_y")  # y:[None,class_num]
        # y:[None,class_num]. this is for multi-label classification only.
        self.input_y_multilabel = tf.placeholder(
            tf.float32, [None, self.class_num], name="input_y_multilabel")
    ...
    def instantiate_weights(self):
        ...
        with tf.name_scope("embedding"):  # embedding matrix
            ...
            self.W_projection = tf.get_variable("W_projection", shape=[
                                                self.filter_num_total, self.class_num], initializer=self.initializer)  # [embed_size,label_size]
            self.b_projection = tf.get_variable(
                "b_projection", shape=[self.class_num])  # [label_size] #ADD 2017.06.09
    ...
    def inference(self):
        ...
        for i, filter_size in enumerate(self.filter_sizes):
            with tf.name_scope("convolution-pooling-%s" % filter_size):
                ...
                filter = tf.get_variable("filter-%s" % filter_size, [
                                         filter_size, self.embed_size, 1, self.filter_num], initializer=self.initializer)
                ...
                # 1)each filter with conv2d's output a shape:[1,sentence_length-filter_size+1,1,1];2)*filter_num--->[1,sentence_length-filter_size+1,1,filter_num];3)*batch_size--->[batch_size,sentence_length-filter_size+1,1,filter_num]
                # input data format:NHWC:[batch, height, width,
                # channels];output:4-D
                # shape:[batch_size,sentence_length - filter_size +
                # 1,1,filter_num]
                conv = tf.nn.conv2d(self.sentence_embeddings_expanded, filter, strides=[
                                    1, 1, 1, 1], padding="VALID", name="conv")
                # ====>c. apply nolinearity
                b = tf.get_variable("b-%s" % filter_size,
                                    [self.filter_num])  # ADD 2017-06-09
                # shape:[batch_size,sentence_length - filter_size +
                # 1,1,filter_num]. tf.nn.bias_add:adds `bias` to `value`
                ...
                # shape:[batch_size, 1, 1, filter_num].max_pool:performs the
                # max pooling on the input.
                pooled = tf.nn.max_pool(h, ksize=[
                                        1, self.sentence_length - filter_size + 1, 1, 1], strides=[1, 1, 1, 1], padding='VALID', name="pool")
        ...
        # shape:[batch_size, 1, 1, filter_num_total]. tf.concat=>concatenates
        # tensors along one dimension.where
        # filter_num_total=filter_num_1+filter_num_2+filter_num_3
        self.h_pool = tf.concat(pooled_outputs, 3)
        # shape should be:[None,filter_num_total]. here this operation has
        # some result as tf.sequeeze().e.g. x's shape:[3,3];tf.reshape(-1,x) &
        # (3, 3)---->(1,9)
        self.h_pool_flat = tf.reshape(
            self.h_pool, [-1, self.filter_num_total])
        ...
        with tf.name_scope("output"):
            # shape:[None,
            # self.class_num]==tf.matmul([None,self.embed_size],[self.embed_size,self.class_num])
            logits = tf.matmul(
    ...
    def loss_multilabel(self, l2_lambda=0.00001):
        with tf.name_scope("loss"):
            # input: `logits` and `labels` must have the same shape `[batch_size, class_num]`
...
# def test():
    ...
    # filter_sizes=[3,4,5]
    # filter_num=128
    #textRNN=TextCNN(filter_sizes,filter_num,class_num, learning_rate, batch_size, decay_steps, decay_rate,sentence_length,vocab_size,embed_size,is_training)
    # with tf.Session() as sess:
    #   sess.run(tf.global_variables_initializer())
    #   for i in range(100):
    #        input_x=np.zeros((batch_size,sentence_length)) #[None, self.sentence_length]
    #        input_x[input_x>0.5]=1
    #       input_x[input_x <= 0.5] = 0
    #       input_y=np.array([1,0,1,1,1,2,1,1])#np.zeros((batch_size),dtype=np.int32) #[None, self.sentence_length]
