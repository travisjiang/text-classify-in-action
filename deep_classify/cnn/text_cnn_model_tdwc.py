...
class TextCnnTDWC:
    """
    this is a TDWC text cnn model, which means input include:
    * title_char
    * title_word
    * desc_char
    * desc_word
    this will be handled as channels in cnn
    """
    def copy_options(options):
        ...
        self.embed_info = options.embed_info
        self.input_type_num = options.input_type_num

        self.filter_total_num = options.filter_num * len(options.filter_sizes)

    def __init__(self, options_tdwc, is_training):
        """init all hyperparameter here"""
        assert(isinstance(options_tdwc, OptionsTDWC))
        self.copy_options(options_tdwc)
        ...
        # inference
        # [None, self.class_num]. main computation graph is here.
    ...
    def add_placeholder(self):
        ...
        for name, (sequence_length, _) in self.data_info.items():
            self.input_x_map[name] = tf.placeholder(
                tf.int32, [None, sequence_length], name="input_x:%s" % name)
    ...
    def instantiate_weights(self):
        ...
        with tf.name_scope("embedding"):
            for name, (vocab_size, embed_size) in self.embed_info.items():
                embed_map[name] = tf.get_variable("embed:%s" % name, shape=[
                                                  vocab_size, embed_size], initializer=self.initializer)

            self.W_projection = tf.get_variable("W_projection",
                                                shape=[self.filter_total_num,
                                                       self.class_num],
    ...
    def inference(self):
        ...
        for name, embedding in self.embed_map:
            embedded_x = tf.nn.embedding_lookup(
                embedding, self.input_x_map[name])
            embedded_list.append(embedded_x)

        # [batch_size, sentence_len, embed_size, 1]
        embedded_list = [tf.expand_dims(e, -1) for e in embedded_list]
        # [batch_size, sentence_len, embed_size, 4]
        embedded_x = tf.concat(embedded_list, 3)

        # NHWC
        height = self.sequence_len
        width = self.embed_size
        in_channels = self.input_type_num
        ...
        for i, filter_size in enumerate(self.filter_sizes):
            with tf.name_scope("convolution-pooling-%s" % filter_size):
                ...
                # [batch_size, height-filter_size+1, 1, out_channels]
                ...
                pooled = tf.nn.max_pool(h,
                                        ksize=[1, height - filter_size + 1, 1, 1],
        ...
        h_pool_flat = tf.reshape(
            h_pool, [-1, self.filter_total_num])
...
# def test():
    ...
    # filter_sizes=[3,4,5]
    # num_filters=128
    #textRNN=TextCNN(filter_sizes,num_filters,class_num, learning_rate, batch_size, decay_steps, decay_rate,sequence_length,vocab_size,embed_size,is_training)
